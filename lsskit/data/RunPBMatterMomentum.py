...
class RunPBMatterMomentum(PowerSpectraLoader):
    """
    Class to return the dark matter momentum terms for the RunPB simulations
    """
        ...
        try:
            ...
            for (ell, ell_prime) in moments:
                ...
                if ell == 0 and ell_prime == 0:
                    d = os.path.join(self.root, 'matter', 'fourier', 'real', 'density', 'power') 
                    ...
                    if los:
                        ...
                        basename = 'poles_P%d%d_runPB_%s_{a}_Nmu5_%slos.dat' %args
                    else:
                        args = (ell, ell_prime, self.tag)
                        basename = 'poles_P%d%d_runPB_%s_{a}_Nmu5.dat' %args
    ...
    def _errors_from_realizations(self, a, moments, sel_mu=None):
        """
        ...
        for i, (ell, ell_prime) in enumerate(moments):
            ...
            basename = 'poles_P%d%d_runPB_*_%s_Nmu5_*los.dat' %args
            ...
            for j, f in enumerate(files):
                ...
                loaded = io.load_momentum(f, ell, ell_prime)
    ...
    def _add_errors(self, poles, moments, save_errors=False, sel_mu=None, name=None, ignore_cache=False):
        ...
        for key in poles.ndindex(dims=['a']):
            ...
            # compute fresh
                ...
                errors = self._errors_from_realizations(key['a'], moments, sel_mu=sel_mu)
                if save_errors:
    ...
    def get_P11_plus_P02(self, los="", save_errors=False, ignore_cache=False):
        """
        Return real space matter P11 + P02
        """
    ...
    def get_P12_plus_P03(self, los="", save_errors=False, ignore_cache=False):
        """
        Return real space matter P12 + P03
        """
        kw = {'los':los, 'save_errors':save_errors, 'ignore_cache':ignore_cache}
        moments = [(1, 2), (0, 3)]
        return self._get_moment(moments, **kw)
    ...
    def get_P13_plus_P22_plus_P04(self, los="", save_errors=False, ignore_cache=False):
        """
        Return real space matter P13 + P22 + P04
        """
        kw = {'los':los, 'save_errors':save_errors, 'ignore_cache':ignore_cache}
        moments = [(1, 3), (2, 2), (0, 4)]
        return self._get_moment(moments, **kw)
    ...
    def get_vel_disp(self, los=""):
        ...
        f = os.path.join(self.root, 'meta/matter_mean_vel_powers.hdf')
        ...
        #
        # values = sigma_sq.reshape((len(self.a),))**0.5
        # toret = xr.DataArray(values, dims=['a'], coords=[self.a])
