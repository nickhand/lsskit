...
def data_pkmu_gausscov(pkmu, mu_bounds, kmin=-np.inf, kmax=np.inf, components=False):
    ...
    data = pkmu.data.copy()
    grid = PkmuGrid.from_structured([pkmu.coords['k'], pkmu.coords['mu']], data)
...
def model_pkmu_gausscov(model, pkmu, mu_bounds, kmin=-np.inf, kmax=np.inf):
    ...
    grid = PkmuGrid([pkmu.coords['k'], pkmu.coords['mu']], k, mu, modes)
...
def compute_pole_covariance(pole_set, 
    ...
    coords = [pole_0.coords['k'], np.asarray(ells, dtype=float)]
...
def compute_pkmu_covariance(pkmu_set, 
    ...
    coords = [pkmu_0.coords['k'], pkmu_0.coords['mu']]
